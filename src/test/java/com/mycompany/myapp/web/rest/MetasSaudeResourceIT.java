...
import static com.mycompany.myapp.domain.MetasSaudeAsserts.*;
...
class MetasSaudeResourceIT {
    ...
    private static final Instant DEFAULT_DATA_LIMITE = Instant.ofEpochMilli(0L);
    private static final Instant UPDATED_DATA_LIMITE = Instant.now().truncatedTo(ChronoUnit.MILLIS);
    ...
    public static MetasSaude createEntity(EntityManager em) {
        MetasSaude metasSaude = new MetasSaude().tipoMeta(DEFAULT_TIPO_META).valorMeta(DEFAULT_VALOR_META).dataLimite(DEFAULT_DATA_LIMITE);
        return metasSaude;
    }
    ...
    public static MetasSaude createUpdatedEntity(EntityManager em) {
        MetasSaude metasSaude = new MetasSaude().tipoMeta(UPDATED_TIPO_META).valorMeta(UPDATED_VALOR_META).dataLimite(UPDATED_DATA_LIMITE);
        return metasSaude;
    }
    ...
    @Test
    @Transactional
    void getAllMetasSaudesByDataLimiteIsEqualToSomething() throws Exception {
        // Initialize the database
        metasSaudeRepository.saveAndFlush(metasSaude);

        // Get all the metasSaudeList where dataLimite equals to
        defaultMetasSaudeFiltering("dataLimite.equals=" + DEFAULT_DATA_LIMITE, "dataLimite.equals=" + UPDATED_DATA_LIMITE);
    }

    @Test
    @Transactional
    void getAllMetasSaudesByDataLimiteIsInShouldWork() throws Exception {
        // Initialize the database
        metasSaudeRepository.saveAndFlush(metasSaude);

        // Get all the metasSaudeList where dataLimite in
        defaultMetasSaudeFiltering(
            "dataLimite.in=" + DEFAULT_DATA_LIMITE + "," + UPDATED_DATA_LIMITE,
            "dataLimite.in=" + UPDATED_DATA_LIMITE
        );
    }

    @Test
    @Transactional
    void getAllMetasSaudesByDataLimiteIsNullOrNotNull() throws Exception {
        // Initialize the database
        metasSaudeRepository.saveAndFlush(metasSaude);

        // Get all the metasSaudeList where dataLimite is not null
        defaultMetasSaudeFiltering("dataLimite.specified=true", "dataLimite.specified=false");
    }
    ...
    void putExistingMetasSaude() throws Exception {
        ...
        updatedMetasSaude.tipoMeta(UPDATED_TIPO_META).valorMeta(UPDATED_VALOR_META).dataLimite(UPDATED_DATA_LIMITE);
    ...
    void partialUpdateMetasSaudeWithPatch() throws Exception {
        ...
        partialUpdatedMetasSaude.dataLimite(UPDATED_DATA_LIMITE);
    ...
    void fullUpdateMetasSaudeWithPatch() throws Exception {
        ...
        partialUpdatedMetasSaude.tipoMeta(UPDATED_TIPO_META).valorMeta(UPDATED_VALOR_META).dataLimite(UPDATED_DATA_LIMITE);
