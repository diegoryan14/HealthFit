...
type RestOf<T extends IMetasSaude | NewMetasSaude> = Omit<T, 'dataLimite'> & {
...
export class MetasSaudeService {
  ...
  protected convertDateFromClient<T extends IMetasSaude | NewMetasSaude | PartialUpdateMetasSaude>(metasSaude: T): RestOf<T> {
    return {
      ...metasSaude,
      dataLimite: metasSaude.dataLimite?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMetasSaude: RestMetasSaude): IMetasSaude {
    return {
      ...restMetasSaude,
      dataLimite: restMetasSaude.dataLimite ? dayjs(restMetasSaude.dataLimite) : undefined,
    };
  }
